...
while run:
    ...
    for event in pygame.event.get():
        ...
        # then storing the current position
            ...
            position = event.pos
            polygon_positions.append(position)
            
    # Using for loop to iterate
    # over the circle_positions
    # list
    if len(polygon_positions) % 3 == 0:
        for i in range(0, len(polygon_positions), 3):
            left_corner = polygon_positions[i]
            right_corner = polygon_positions[i + 1]
            # Drawing the circle
            pygame.draw.polygon(window, color, [left_corner, right_corner, polygon_positions[i+2]])
